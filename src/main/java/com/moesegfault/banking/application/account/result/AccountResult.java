...
/**
 * @brief 账户查询结果（Account Query Result），对外暴露稳定账户视图；
 *        Stable account view returned by account query use cases.
 */
public final class AccountResult {
    ...
    /**
     * @brief 关户时间（Closed Timestamp），对齐 `account.closed_at`；
     *        Closed timestamp aligned with `account.closed_at`.
     */
    ...
    /**
     * @brief 构造账户查询结果（Construct Account Result）；
     *        Construct account query result.
     *
     ...
     * @param closedAt                关户时间（Closed timestamp）。
    ...
    /**
     * @brief 从基础账户构建结果（Build Result from Base Account）；
     *        Build result from base account.
     *
     * @param account 账户实体（Account entity）。
     * @return 账户结果（Account result）。
     */
    public static AccountResult from(final Account account) {
        return from(account, null);
    }

    /**
     * @brief 从账户与绑定储蓄账户构建结果（Build Result with Linked Savings Account）；
     *        Build result from account and linked savings-account id.
     *
     * @param account                 账户实体（Account entity）。
     * @param linkedSavingsAccountId 绑定储蓄账户 ID（Linked savings-account ID, nullable）。
     * @return 账户结果（Account result）。
     */
    public static AccountResult from(final Account account, final SavingsAccountId linkedSavingsAccountId) {
    ...
    /**
     * @brief 返回关户时间（Return Closed Timestamp）；
     *        Return closed timestamp.
     *
     * @return 关户时间（Closed timestamp, nullable）。
     */
