...
/**
 * @brief 开外汇账户处理器（Open FX Account Handler）；
 *        Application handler that orchestrates FX-account opening.
 */
public final class OpenFxAccountHandler {
    ...
    /**
     * @brief 执行开外汇账户（Handle Open FX Account）；
     *        Execute FX-account opening orchestration.
     *
     * @param command 开外汇账户命令（Open FX-account command）。
     * @return 开户结果（Open-account result）。
     */
    public OpenAccountResult handle(final OpenFxAccountCommand command) {
        final OpenFxAccountCommand normalized = Objects.requireNonNull(command, "command must not be null");
        return transactionManager.execute(() -> {
            ...
            final SavingsAccount linkedSavingsAccount = AccountApplicationSupport.loadSavingsAccountOrThrow(
                    accountRepository,
                    normalized.linkedSavingsAccountId());
            final FxAccount fxAccount = FxAccount.open(
                    FxAccountId.of(idGenerator.nextId()),
                    CustomerId.of(customer.customerId().value()),
                    accountNo,
                    linkedSavingsAccount.savingsAccountId());

            AccountPolicy.ensureFxLinkedSavingsOwnership(fxAccount, linkedSavingsAccount);
            accountRepository.saveFxAccount(fxAccount);
            eventPublisher.publish(fxAccount.openedEvent());
            return OpenAccountResult.from(fxAccount.account(), linkedSavingsAccount.savingsAccountId());
        });
    }
}
