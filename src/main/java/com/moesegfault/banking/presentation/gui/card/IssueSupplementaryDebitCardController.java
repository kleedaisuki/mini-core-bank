...
/**
 * @brief 借记附属卡发卡控制器（Issue Supplementary Debit Card Controller），处理事件并调用应用层发卡服务；
 *        Controller for supplementary debit-card issuance that handles view events and invokes application handler.
 */
public final class IssueSupplementaryDebitCardController implements GuiController {

    /**
     * @brief 页面模型（Page Model）；
     *        Bound page model.
     */
    private final IssueSupplementaryDebitCardModel model;
     ...
     *        Construct issue-supplementary-debit-card controller.
     *
     ...
     */
    public IssueSupplementaryDebitCardController(
            final IssueSupplementaryDebitCardModel model,
     ...
     *        Submit supplementary debit-card issuance request.
     *
    ...
    private void submit(final Map<String, Object> attributes) {
        ...
        try {
            final IssueCardResult result = applicationHandler.handle(new IssueSupplementaryDebitCardCommand(
                    ...
                    model.fieldValue(CardGuiSchema.PRIMARY_DEBIT_CARD_ID),
    ...
    private Map<String, String> extractValues(final Map<String, Object> attributes) {
        ...
        return Map.of(
                ...
                valueOrModel(normalizedAttributes, CardGuiSchema.PRIMARY_DEBIT_CARD_ID),
