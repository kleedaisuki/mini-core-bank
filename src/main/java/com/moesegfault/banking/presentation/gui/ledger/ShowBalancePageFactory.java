...
/**
 * @brief 余额查询页面工厂（Show Balance Page Factory），创建余额查询页面 MVC 三件套；
 *        Balance-query page factory creating model/view/controller bundle.
 */
public final class ShowBalancePageFactory implements GuiPageFactory {
    ...
    /**
     * @brief 构造余额查询页面工厂（Construct Show Balance Page Factory）；
     *        Construct balance-query page factory.
     *
     ...
     */
    public ShowBalancePageFactory(final FindBalanceHandler applicationHandler,
    ...
    public GuiPage createPage(final GuiContext context) {
        ...
        final ShowBalanceController controller = new ShowBalanceController(model, applicationHandler, guiExceptionHandler);
        final ShowBalanceView view = new ShowBalanceView(
                guiRuntime,
                controller,
        ...
        return new GuiPage(LedgerGuiPageIds.SHOW_BALANCE, model, view, controller);
    }
