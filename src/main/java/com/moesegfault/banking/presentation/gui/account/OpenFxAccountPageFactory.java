...
/**
 * @brief 开立外汇账户页面工厂（Open FX Account Page Factory），创建外汇开户页 MVC 组合；
 *        Open-FX-account page factory that creates MVC bundle for FX opening page.
 */
public final class OpenFxAccountPageFactory implements GuiPageFactory {
    ...
    /**
     * @brief 构造页面工厂（Construct Page Factory）;
     *        Construct open-FX-account page factory with full dependencies.
     *
     * @param applicationHandler 开外汇账户应用服务（Application handler）。
     * @param formViewSupplier 表单视图提供器（Form-view supplier）。
    ...
    public GuiPage createPage(final GuiContext context) {
        ...
        final OpenFxAccountModel model = new OpenFxAccountModel();
        normalizedContext.currentCustomerId().ifPresent(customerId -> model.setFormValues(customerId, "", ""));
        final OpenFxAccountController controller = new OpenFxAccountController(
                applicationHandler,
                model,
                exceptionHandler);
        final OpenFxAccountView view = new OpenFxAccountView(
                Objects.requireNonNull(formViewSupplier.get(), "formViewSupplier must not return null"),
                errorDialogView,
                successDialogView,
                controller::onViewEvent);
        view.bindModel(model);
        return new GuiPage(AccountGuiPageIds.OPEN_FX_ACCOUNT, model, view, controller);
    }
}
