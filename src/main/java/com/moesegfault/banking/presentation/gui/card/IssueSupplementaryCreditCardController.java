...
/**
 * @brief 信用附属卡发卡控制器（Issue Supplementary Credit Card Controller），处理事件并调用应用层发卡服务；
 *        Controller for supplementary credit-card issuance that handles view events and invokes application handler.
 */
public final class IssueSupplementaryCreditCardController implements GuiController {

    /**
     * @brief 页面模型（Page Model）；
     *        Bound page model.
     */
    private final IssueSupplementaryCreditCardModel model;
     ...
     *        Construct issue-supplementary-credit-card controller.
     *
     ...
     */
    public IssueSupplementaryCreditCardController(
            final IssueSupplementaryCreditCardModel model,
     ...
     *        Submit supplementary credit-card issuance request.
     *
    ...
    private void submit(final Map<String, Object> attributes) {
        ...
        try {
            final IssueCardResult result = applicationHandler.handle(new IssueSupplementaryCreditCardCommand(
                    ...
                    model.fieldValue(CardGuiSchema.PRIMARY_CREDIT_CARD_ID),
    ...
    private Map<String, String> extractValues(final Map<String, Object> attributes) {
        ...
        return Map.of(
                ...
                valueOrModel(normalizedAttributes, CardGuiSchema.PRIMARY_CREDIT_CARD_ID),
