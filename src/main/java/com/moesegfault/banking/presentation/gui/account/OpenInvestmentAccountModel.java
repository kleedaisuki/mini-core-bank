...
/**
 * @brief 开立投资账户页面模型（Open Investment Account Model），维护投资开户输入与提交流程状态；
 *        Open-investment-account page model storing investment-open input and submission lifecycle.
 */
public final class OpenInvestmentAccountModel extends AbstractGuiModel {
     ...
     *        Update customer-id and account-number form values.
     *
     * @param customerId 客户 ID（Customer ID）。
     * @param accountNo 账户号（Account number）。
     */
    public void setFormValues(final String customerId, final String accountNo) {
        this.customerId = normalizeNullable(customerId);
        this.accountNo = normalizeNullable(accountNo);
        fireChanged(AccountGuiSchema.CUSTOMER_ID, AccountGuiSchema.ACCOUNT_NO);
    }
