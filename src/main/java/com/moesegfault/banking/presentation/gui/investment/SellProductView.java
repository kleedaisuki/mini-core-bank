...
/**
 * @brief 卖出投资产品页面视图（Sell Product Page View）；
 *        Toolkit-agnostic view adapter for sell-product page with event emission helpers.
 */
public final class SellProductView implements GuiView<SellProductModel> {

    /**
     * @brief 绑定模型（Bound Model）；
     *        Bound page model.
     */
    private SellProductModel model;
     ...
     */
    @Override
    public void bindModel(final SellProductModel model) {
    ...
    @Override
    public void render() {
        final SellProductModel boundModel = Objects.requireNonNull(model, "model must be bound before render");
